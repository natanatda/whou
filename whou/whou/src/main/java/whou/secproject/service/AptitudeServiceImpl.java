...
public class AptitudeServiceImpl implements AptitudeService {
	...
	public AptitudeTestValueDTO testCrawling(String testURL, String qnum) {
		...
		if (qnum.equals("25")) {
			...
			// 가치 요약 정보 가져오기
			for(int i = 0; i <3; i++) 
				sb.append(plus.elementToSb(driver, tagList[i], sb));
	...
	public List<String> reportView(String qnum, AptitudeTestValueDTO dto) {
		...
		if (qnum.equals("27")) {
			result = Stream
					.of(dto.getTest27_1(), dto.getTest27_2(), dto.getTest27_3(), dto.getTest27_4() + dto.getTest27_5())
					.flatMap(value -> {
					...
					}).map(value -> value.replace("검사결과", "")).map(String::trim) // 공백 제거
					.filter(value -> value != null && !value.isEmpty()).collect(Collectors.toList());
			System.out.println("27의 결과 리스트" + result);
		}

		if (qnum.equals("25")) {
			result = Stream.of(dto.getTest25_1(), dto.getTest25_2(), dto.getTest25_3(), dto.getTest25_4())
					.flatMap(value -> {
