...
public class AptitudeServiceImpl implements AptitudeService{
	...
	public AptitudeTestValueDTO testCrawling(String testURL, String qnum) {
		...
		driver.get(testURL);
		CrawingResult plus = new CrawingResult(){@Override
        ...
        if(qnum.equals("27")) {
        	...
        	CrawingResult develop = new CrawingResult(){@Override
        	    public StringBuilder replSb(StringBuilder sb, WebElement element) {
        	...
        	CrawingResult score = new CrawingResult(){@Override
        		public StringBuilder replSb(StringBuilder sb, WebElement element) {
        ...
        if(qnum.equals("25")) {
	        ...
	        // 가치 요약 정보 가져오기
        	tag = "div.aptitude-tbl-list.value.import > table > tbody > tr:nth-child(1) > td:nth-child(2)";
        	sb.append(plus.elementToSb(driver,tag,sb));
        	tag = "div.aptitude-tbl-list.value.import > table > tbody > tr:nth-child(2) > td:nth-child(1)";
        	sb.append(plus.elementToSb(driver,tag,sb));
        	tag = "div.aptitude-tbl-list.value.import > table > tbody > tr:nth-child(2) > td.left.center.me";
        	sb.append(plus.elementToSb(driver,tag,sb));
            dto.setTest25_1(sb.toString());
        	sb.delete(0, sb.length());

            // 가치 점수 정보 가져오기
            tag = "#ct > div.aptitude_result_wrap > div.aptitude-result-content > div:nth-child(1) > div:nth-child(5) > table > tbody > tr > td:nth-child(3n+0)";
            dto.setTest25_2(plus.elementToSb(driver,tag,sb).toString());
        	sb.delete(0, sb.length());

            // 가치 유형 정보 가져오기
            tag = "div.aptitude-result-content > div:nth-child(1) > div:nth-child(8) > table > tbody > tr:nth-child(2) > th";
            dto.setTest25_3(plus.elementToSb(driver,tag,sb).toString());
        	sb.delete(0, sb.length());
        	
            // value_import 정보 가져오기
            for (int i = 1; i <= 3; i++) {
                tag = "div.aptitude-tbl-list.value.best > table > tbody > tr:nth-child(2) > td:nth-child(" + i + ")";
                sb = plus.elementToSb(driver,tag,sb);
            }
        ...
        if (qnum.equals("21")) {
        	String[] tagList = {
        		    "div.cont-wrap.page-break > ul > li > strong", // TOP3
        	...
        	try {TimeUnit.SECONDS.sleep(1); // 3초 동안 대기
        	...
        	for(int i = 0; i < 6; i++) {
        		if(i==0)dto.setTest21_1(valueList.get(i));
        		if(i==1)dto.setTest21_2(valueList.get(i));
        		if(i==2)dto.setTest21_3(valueList.get(i));
        		if(i==3)dto.setTest21_4(valueList.get(i));
        		if(i==4)dto.setTest21_5(valueList.get(i));
        		if(i==5)dto.setTest21_6(valueList.get(i));
        	}
        ...
        if (qnum.equals("31")) {
        	...
        	for(int i = 0; i < 6; i++) {
        		if(i==0)dto.setTest31_1(valueList.get(i));
        		if(i==1)dto.setTest31_2(valueList.get(i));
        		if(i==2)dto.setTest31_3(valueList.get(i));
        		if(i==3)dto.setTest31_4(valueList.get(i));
        		if(i==4)dto.setTest31_5(valueList.get(i));
        		if(i==5)dto.setTest31_6(valueList.get(i));
        	}
         }
