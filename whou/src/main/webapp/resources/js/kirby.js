 ...
 */ 
 ...
 import { GLTFLoader } from 'https://cdn.skypack.dev/three@0.129.0/examples/jsm/loaders/GLTFLoader.js';
    ...
    const object = new THREE.Object3D();
        ...
        // 카메라 위치 조정 ( 작을수록 가까워짐 )
        camera.position.set(kirbyCamera,5,10);
        scene.add(camera)
        ...
        class Figure {
            ...
            createMouth() {
                ...
                const texture = textureLoader.load('../resources/img/mouth.png'); // 입 이미지 텍스처
            ...
            createArms() {
                ...
                for(let i = 0; i < 2; i++) {
                    ...
                    if(i === 1){
						...
						loader.load(kirbyPath, function(gltf){
						...
						if(kirbyColor != null){
							model.traverse(function(child) {
            					...
            					material.color.set(kirbyColor); // 색상 변경
            					material.needsUpdate = true; // 변경 사항 업데이트
        					}
    					});
					}
   						model.scale.set(kirbyScale_x, kirbyScale_y, kirbyScale_z); // x, y, z 크기 조절
	    				model.position.set(kirbyPosition_x, kirbyPosition_y, kirbyPosition_z); // x = 좌우, y = 위아래, z = 앞뒤 이동
						model.rotation.set(kirbyRotation_x, kirbyRotation_y, kirbyRotation_z);
				    	if(kirbyMotion == 0){
				    		scene.add(model);
						}else{
							arm.add(model);
          	...
          	createEyes() {
                ...
                const textureLoader = new THREE.TextureLoader();
                const texture = textureLoader.load('../resources/img/eye.png'); // 단색 텍스처 예시
